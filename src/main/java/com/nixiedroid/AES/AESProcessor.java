...
final class AESProcessor {
    ...
    public AESProcessor(byte[] key, boolean flag) {
       init(key, flag);
    }


    private void init(byte[] key,boolean flag){
        int KC = key.length >>> 2;
        if (rounds == 0) rounds = KC + 6;//Not always true
        int roundCounter = (rounds + 1) * 4;
        int roundCirclePointer = 0;
        int tt;

        //Create arrays for key expansion
        decryptKey = new int[rounds + 1][];
        encryptKey = new int[rounds + 1][];
        for (int i = 0; i <= rounds; i++) {
            decryptKey[i] = new int[]{0, 0, 0, 0};
        }
        for (int i = 0; i <= rounds; i++) {
            encryptKey[i] = new int[]{0, 0, 0, 0};
        }

        //Split key to 4-byte integers
        int[] keyInts = new int[key.length>>>2];
        for (int i = 0; i < key.length>>>2; i++) {
            byte[] arr = new byte[key.length>>>2];
            System.arraycopy(key, i * 4, arr, 0, 4);
            keyInts[i] = (arr[3] & 0xFF)       |
                         (arr[2] & 0xFF) << 8  |
                         (arr[1] & 0xFF) << 16 |
                         (arr[0] & 0xFF) << 24 ;
            }
        ...
        for (int i = 0; i < KC; i++) {
            encryptKey[          i >>> 2  ][ i % 4 ] = keyInts[i];
            decryptKey[rounds - (i >>> 2) ][ i % 4 ] = keyInts[i];
        }
        int t = KC;
        while (t < roundCounter) {
            tt = keyInts[KC - 1];
            keyInts[0] ^= 
			(((C.S [ (tt >>>  16) & 0xFF ] << 24) & 0xff000000) ^
             ((C.S [ (tt >>>  8 ) & 0xFF ] << 16) & 0xff0000  ) ^
             ((C.S [  tt          & 0xFF ] <<  8) & 0xff00    ) ^
             ((C.S [ (tt >>>  24) & 0xFF ]      ) & 0xff      ) ^
              (C.rcon[roundCirclePointer ] << 24) & 0xff000000) ;
            roundCirclePointer++;

            //For keys <256 bit. Or >256
            ...
            //For 256 bit key:
                ...
                keyInts[KC >>> 1] ^= 
				(((C.S [ ( tt         & 0xFF ) ]      ) & 0xff      ) ^
                 ((C.S [ ((tt >>>  8) & 0xFF ) ] <<  8) & 0xff00    ) ^
                 ((C.S [ ((tt >>> 16) & 0xFF ) ] << 16) & 0xff0000  ) ^
                  (C.S [  (tt >>> 24)          ] << 24) & 0xff000000) ;


                for (int i =((KC >>> 1) + 1); i < KC; i++) {
                    keyInts[i] ^= keyInts[i - 1];
                }
            }
            ...
            int j = 0;
            while ((j < KC) && (t < roundCounter)) {
                encryptKey[          t >>> 2  ][ t % 4 ] = keyInts[j];
                decryptKey[rounds - (t >>> 2) ][ t % 4 ] = keyInts[j];
                j++;
                t++;
            }
        }
        ...
        if (flag) {
            decryptKey[rounds - 4][0] ^=( ( (byte) (0x73) << 24) & 0xff000000);
            decryptKey[rounds - 6][0] ^=( ( (byte) (0x09) << 24) & 0xff000000);
            decryptKey[rounds - 8][0] ^=( ( (byte) (0xe4) << 24) & 0xff000000);
            encryptKey[         4][0] ^=( ( (byte) (0x73) << 24) & 0xff000000);
            encryptKey[         6][0] ^=( ( (byte) (0x09) << 24) & 0xff000000);
            encryptKey[         8][0] ^=( ( (byte) (0xe4) << 24) & 0xff000000);
        }
    ...
    public byte[] decrypt(byte[] data) {
        int rounds = decryptKey.length -1;
        int[] t = new int[4];
        int[] a = new int[4];
        int s1 = 3;
        int s2 = 2;
        ...
        for (int i = 0; i < 4; i++) {
            byte[] arr = new byte[4];
            System.arraycopy(data,i*4,arr,0,4);
            t[i] = compactWord(arr)^ decryptKey[0][i];
        }
        for (int i = 1; i < rounds; i++) {
            for (int j = 0; j < 4; j++) {
                a[j] = 
				((C.T5 [  (t [ j          ] >>>  24)          ]) ^
                 (C.T6 [ ((t [(j + s1) % 4] >>>  16) & 0xff ) ]) ^
                 (C.T7 [ ((t [(j + s2) % 4] >>>   8) & 0xff ) ]) ^
                 (C.T8 [ ((t [(j + s3) % 4]        ) & 0xff ) ]) ^
                decryptKey[i][j]);
            }
        ...
        int tt;
        for (int i = 0; i < 4; i++) {
            tt = decryptKey[rounds][i];
            result[4*i]   = ((byte)  ((C.Si [ ((t [ i         ] >>  24) & 0xff)]^ (tt >> 24)) & 0xFF));
            result[4*i+1] = ((byte)  ((C.Si [ ((t [(i+s1)  %4 ] >>  16) & 0xff)]^ (tt >> 16)) & 0xFF));
            result[4*i+2] = ((byte)  ((C.Si [ ((t [(i+s2)  %4 ] >>   8) & 0xff)]^ (tt >> 8 )) & 0xFF));
            result[4*i+3] = ((byte)  ((C.Si [ ((t [(i+s3)  %4 ]       ) & 0xff)]^  tt       ) & 0xFF));
        }
        return result;
    }
    public byte[] encrypt(byte[] data){
        int rounds = encryptKey.length -1;
        int[] t = new int[4];
        int[] a = new int[4];
        int s1 = 1;
        int s2 = 2;
        ...
        for (int i = 0; i < 4; i++) {
            byte[] arr = new byte[4];
            System.arraycopy(data,i*4,arr,0,4);
            t[i] = compactWord(arr)^ encryptKey[0][i];
        }
        for (int i = 1; i < rounds; i++) {
            for (int j = 0; j < 4; j++) {
                a[j] = 
				((C.T1 [  ( t[ j           ] >>>  24)          ]) ^
                 (C.T2 [ (( t[(j + s1) % 4 ] >>>  16) & 0xff ) ]) ^
                 (C.T3 [ (( t[(j + s2) % 4 ] >>>   8) & 0xff ) ]) ^
                 (C.T4 [ (( t[(j + s3) % 4 ]        ) & 0xff ) ]) ^
                        encryptKey[i][j]);
            }
            System.arraycopy(a,0,t,0,a.length);
        ...
        int tt;
        for (int i = 0; i < 4; i++) {
            tt = encryptKey[rounds][i];
            result[4*i]   = ((byte)  ((C.S [ ((t [ i         ] >>  24) & 0xff)]^ (tt >> 24)) & 0xFF));
            result[4*i+1] = ((byte)  ((C.S [ ((t [(i+s1)  %4 ] >>  16) & 0xff)]^ (tt >> 16)) & 0xFF));
            result[4*i+2] = ((byte)  ((C.S [ ((t [(i+s2)  %4 ] >>   8) & 0xff)]^ (tt >>  8)) & 0xFF));
            result[4*i+3] = ((byte)  ((C.S [ ((t [(i+s3)  %4 ]       ) & 0xff)]^  tt       ) & 0xFF));
        }
        return result;
    }
    private static int compactWord(byte[] data) {
        return  ((data[0] & 0xFF) << 24 ) & 0xff000000 |
                ((data[1] & 0xFF) << 16 ) & 0xff0000   |
                ((data[2] & 0xFF) << 8  ) & 0xff00     |
                ((data[3] & 0xFF)       ) & 0xff       ;
    }
